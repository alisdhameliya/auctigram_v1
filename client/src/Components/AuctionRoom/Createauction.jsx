...
import urlUploadVideo from "../../../public/videos/urlUploadVideo.webm";
...
const Createauction = ({ show, setshow }) => {
  ...
  const clickHandler = async () => {
    if (!url || !title || !date || !fileref.current?.files[0] || !desc) {
    ...
    try {
      const response = await axios.post(`${SERVER_URL}/create/room`, formData, {
        headers: {
      ...
      alert("There was an error creating the auction room. Please try again.");
    }
  };
        ...
        <div className="text-white flex flex-wrap gap-5 w-full justify-between">
          ...
          <div className="border w-full rounded-md sm:min-w-72 max-w-full bg-[#bec0dd0d] flex justify-center items-center flex-1">
            <div className="flex flex-col gap-2 w-full px-6 py-4">
              ...
              <div className="relative bg-[#062541] rounded-full px-10 py-5 w-full flex items-center">
                <input
                  type="file"
                  ref={fileref}
                  className="absolute inset-0 w-full h-full file:h-0 file:w-0 pt-1 file:opacity-0 cursor-pointer "
                />
