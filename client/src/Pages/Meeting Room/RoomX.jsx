...
const RoomPage = () => {
  ...
  const myMeeting = async (element) => {
    const appID = 1672527728;
    const serverSecret = "aedd6607bdff6cc0f7b0fd9657134295";
    ...
    >
      ...
      {showContent && (
        <div className="absolute z-[54545545] top-0  p-4 rounded-2xl flex gap-4 flex-wrap">
          <div
            className={`left shadow-2xl h-fit bg-white  rounded-xl transition-all duration-1000 relative ${showBidSection ? "flex flex-col gap-4 items-center " : ""
              }`}
            style={{ padding: "0.5rem 1.5rem 0.5rem 1.5rem" }}
          >
            <IoIosArrowDown
              className={`absolute right-4 top-5 transition-all duration-1000 cursor-pointer active:scale-95  ${showBidSection ? "rotate-180" : ""
                }`}
              size="1.5rem"
              onClick={() => setShowBidSection(!showBidSection)}
            />
            <div
              className={`z-[545454545] w-40 rounded-xl min-w-fit flex  items-center justify-center gap-2 bg-white transition-all duration-500  ${showAnimation ? "wiggleAnimation" : ""
            ...
            >
              ...
              <div
                className={`name text-xs pb-4 text-gray-400 transition-all duration-1000 ${showBidSection ? "" : "hidden"
                  }`}
              >
            ...
            <input
              type="number"
              className={`rounded-xl p-2 outline-none bg-transparent text-white ${showBidSection ? "" : "hidden"
                }`}
          ...
          <div className="flex flex-col gap-4 items-center justify-center h-fit">
            <button
              className={`right bg-red-500  text-2xl rounded-xl font-bold text-white p-4 h-fit  ${host ? "" : "hidden"
                }`}
              onClick={closeRoom}
            >
              End Auction
            ...
            <div
              className={`creditsLeft bg-white rounded-xl p-4 flex items-center gap-2 ${Number(userCreditsLeft) >= 0 ? "text-black" : "text-red-500"
                }`}
